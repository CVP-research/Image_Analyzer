...
class Config:
    ...
    OUTPUT_ROOT = "/home/rocknroll1397/Image_Analyzer/new/dataset/train_veo3_v2" # Version 2 output
    
    TARGET_SIZE = 1024 # 변경 없음 (원본과 동일)
    UPSCALE_FACTOR = 4 # V2: 배경 업스케일링 팩터
...
class DatasetBuilder:
    ...
    def _get_upscaled_backgrounds(self, target_count):
        """256x256 배경을 4배 업스케일링하여 리스트로 반환합니다."""
        ...
        new_size = (self.cfg.TARGET_SIZE, self.cfg.TARGET_SIZE)

        print(f"🖼️ 배경 {target_count}개 로드 및 {self.cfg.UPSCALE_FACTOR}x 업스케일링 중...")
        for path in tqdm(required_paths, desc="Upscaling backgrounds"):
            img = cv2.imread(path)
            if img is not None:
    ...
    def generate_datasets(self):
        ...
        if self.object_files:
            positive_bgs = self._get_upscaled_backgrounds(self.cfg.NUM_BG_GROUPS)
        ...
        if self.cfg.NUM_NEGATIVE_SAMPLES > 0:
            ...
            negative_bgs = self._get_upscaled_backgrounds(negative_bg_count)
