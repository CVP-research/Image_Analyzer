...
# 🚨 사용자 설정 필요 🚨
INPUT_FRAMES_DIR = Path("./data/77_7859_15670/images_real") 
MASKED_FRAMES_DIR = Path("./output/masked_frames_real")
...
async def save_object(frame_id: int, mask_index: int):
    ...
    output_filename = f"{cache['frame_name']}_{mask_index:02d}_mask.png"
    output_path = MASKED_FRAMES_DIR / output_filename
    
    try:
        # 이진 마스크를 0 또는 255 값을 가지는 그레이스케일 이미지로 변환하여 저장
        # 마스크는 이미 uint8이므로 바로 255를 곱하여 저장 가능한 형태로 만듭니다.
        mask_to_save = (selected_mask * 255).astype(np.uint8)
...
if __name__ == "__main__":
    # `uvicorn new.segment_server_real:app --reload` 명령어로 실행하는 것을 권장
    print("--- Uvicorn 서버 직접 실행 ---")
    uvicorn.run("segment_server_real:app", host="0.0.0.0", port=8000, log_level="info", reload=True)