...
def select_job() -> Path:
    ...
    if existing_jobs:
        print("\n기존 jobs:")
        for idx, job_dir in enumerate(existing_jobs, 1):
            # job 폴더 내 파일 수 확인
            file_count = sum(1 for _ in job_dir.rglob("*") if _.is_file())
            mtime = datetime.fromtimestamp(job_dir.stat().st_mtime)
            print(f"  [{idx}] {job_dir.name}  ({file_count} files, {mtime.strftime('%Y-%m-%d %H:%M')})")
    else:
        print("\n기존 jobs가 없습니다.")
    ...
    while True:
        try:
            choice = input("선택 (번호 또는 'new'): ").strip().lower()

            if choice in ['0', 'new', 'n', '']:
                # 새 job 생성
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                new_job_dir = JOBS_DIR / timestamp
                new_job_dir.mkdir(parents=True, exist_ok=True)

                # 하위 폴더 구조 생성
                (new_job_dir / "input").mkdir(exist_ok=True)
                (new_job_dir / "output").mkdir(exist_ok=True)
                (new_job_dir / "output" / "video").mkdir(exist_ok=True)
                (new_job_dir / "output" / "frames").mkdir(exist_ok=True)
                (new_job_dir / "output" / "masked_frames").mkdir(exist_ok=True)
                (new_job_dir / "output" / "dataset").mkdir(exist_ok=True)

                print(f"\n✓ 새 job 생성: {new_job_dir.name}")
                print(f"  → input/ 폴더에 이미지를 넣어주세요.")
                return new_job_dir

            idx = int(choice)
            if 1 <= idx <= len(existing_jobs):
                selected = existing_jobs[idx - 1]
                print(f"\n✓ 선택된 job: {selected.name}")
                return selected
            else:
                print(f"잘못된 선택입니다. 0-{len(existing_jobs)} 사이의 숫자를 입력하세요.")

        except ValueError:
            print("숫자 또는 'new'를 입력하세요.")
...
# =============================================================================
# 파일명 패턴 (pipeline/file_patterns.py에서 관리)
# =============================================================================
from file_patterns import FilePatterns


# =============================================================================
# 의존성 체크 통과 후 나머지 import
# =============================================================================
import os
import random
import time
...
# 디버그 모드가 아닐 때만 무거운 모듈 import
if not _DEBUG_EARLY:
    from google import genai
    from google.genai import types

    from modules.semantic_matcher import SemanticMatcher
    from modules.composite import composite_on_segment, compute_segment_averaged_depth
    from modules.utils import frame_from_video, get_all_objects, load_camera_poses, compute_center, compute_azimuth_elevation, _fixed_grid_selection, _robust_greedy_selection
    from modules.depth import compute_depth
    from modules.segment import get_sam_model
    from modules.yolo_utils import (
        save_yolo_annotation,
        create_yolo_dataset_structure,
        split_train_val
    )
    from modules.logging_utils import setup_logger
    from pose_estimator_ui import run_pose_estimator, check_poses_exist
else:
    # 디버그 모드용 더미 함수들
    def run_pose_estimator(job_dir, auto_open=True):
        print("[DEBUG] run_pose_estimator 스킵됨")
        return {}

    def check_poses_exist(job_dir):
        poses_file = job_dir / "input" / "pose" / "camera_poses.json"
        return poses_file.exists()

    def get_all_objects(original_input_dir, masked_frames_dir):
        """디버그용: 마스크 파일 목록 반환"""
        objects = []
        if masked_frames_dir.exists():
            for mask_path in masked_frames_dir.glob("*.png"):
                objects.append({
                    "path": mask_path,
...
# Job 기반 경로 (초기값, setup_job_paths에서 재설정)
JOB_DIR: Path = None
INPUT_DIR: Path = None
VIDEO_DIR: Path = None
OUTPUT_DIR: Path = None
MASKED_FRAMES_DIR: Path = None
POSE_DIR: Path = None
POSES_FILE: Path = None


def setup_job_paths(job_dir: Path):
    """Job 디렉토리 기반으로 경로 설정"""
    global JOB_DIR, INPUT_DIR, VIDEO_DIR, OUTPUT_DIR, MASKED_FRAMES_DIR, POSE_DIR, POSES_FILE

    JOB_DIR = job_dir
    INPUT_DIR = job_dir / "input"
    VIDEO_DIR = job_dir / "output" / "video"
    OUTPUT_DIR = job_dir / "output" / "dataset"
    MASKED_FRAMES_DIR = job_dir / "output" / "masked_frames"
...
# Generative AI 클라이언트 초기화 (디버그 모드에서는 스킵)
client = None
if not _DEBUG_EARLY:
    try:
...
def generate_360_video_with_veo3(selected_images: List[Path]) -> Path:
    ...
    try:
        ...
        if operation.response and operation.result.generated_videos:
            ...
            local_path = VIDEO_DIR / FilePatterns.VIDEO
...
def segment_objects(frame_dir: Path) -> None:
    ...
    for idx, frame_path in enumerate(frames):
        ...
        out_path = MASKED_FRAMES_DIR / FilePatterns.masked_name(frame_path.stem)
...
def process_single_composite(
    ...
    try:
        ...
        # 파일 저장
        output_filename = FilePatterns.composite_name(split_name, bg_idx, original_idx)
...
def run_full_pipeline(
    ...
    mode_str = " [DEBUG]" if DEBUG_MODE else ""
    print("=" * 60)
    print(f"Few-shot Semantic Compositing Pipeline{mode_str}")
    ...
    if len(input_images) == 0:
        ...
        return []
    print(f"[Step 1] Found {len(input_images)} input images")

    # Step 1.5: 카메라 포즈 등록 (없으면 UI 실행)
    if check_poses_exist(JOB_DIR):
        print(f"\n[Step 1.5] SKIP - 기존 카메라 포즈 사용")
    elif DEBUG_MODE:
        print(f"\n[Step 1.5] DEBUG - 카메라 포즈 등록 스킵 (더미 데이터 생성)")
        # 더미 포즈 생성
        import json
        poses_file = JOB_DIR / "input" / "pose" / "camera_poses.json"
        poses_file.parent.mkdir(parents=True, exist_ok=True)
        dummy_poses = {img.name: {"position": [0, 5, 10], "look_at": [0, 0, 0], "up": [0, 1, 0]} for img in input_images}
        with open(poses_file, 'w') as f:
            json.dump(dummy_poses, f, indent=2)
        print(f"  ✓ 더미 포즈 {len(dummy_poses)}개 생성됨")
    else:
        print(f"\n[Step 1.5] 카메라 포즈 등록 필요")
        print("  Gradio UI가 실행됩니다. 브라우저에서 포즈를 등록하세요.")
        print("  완료 후 '완료 & 파이프라인 계속' 버튼을 클릭하세요.")
        run_pose_estimator(JOB_DIR, auto_open=True)
        print("  ✓ 카메라 포즈 등록 완료")

    # Step 1.8: 뷰 선택
    if DEBUG_MODE:
        print(f"\n[Step 1.8] DEBUG - 뷰 선택 스킵 (첫 {num_views}개 사용)")
        selected_views = input_images[:num_views]
    else:
        selected_views = select_best_views(input_images, num_views=num_views)

    # Step 2: Veo3 360도 영상 생성 (이미 영상이 있으면 건너뛰기)
    video_path = VIDEO_DIR / FilePatterns.VIDEO
    frames_dir = JOB_DIR / "output" / "frames"
    if video_path.exists():
        print(f"\n[Step 2] SKIP - 기존 영상 사용: {video_path.name}")
    elif DEBUG_MODE:
        print(f"\n[Step 2] DEBUG - Veo3 API 호출 스킵")
        print(f"  실제 실행 시: Veo3로 360도 영상 생성")
        # 더미 비디오는 생성하지 않음
    else:
        video_path = generate_360_video_with_veo3(selected_views)

    # Step 2.5: 프레임 분리 (이미 프레임이 있으면 건너뛰기)
    existing_frames = list(frames_dir.glob(FilePatterns.FRAME_GLOB))
    if existing_frames:
        print(f"\n[Step 2.5] SKIP - 기존 프레임 {len(existing_frames)}개 사용")
    elif DEBUG_MODE:
        print(f"\n[Step 2.5] DEBUG - 프레임 분리 스킵")
        print(f"  실제 실행 시: 영상에서 프레임 추출")
        # 입력 이미지를 프레임 디렉토리에 복사 (디버그용)
        frames_dir.mkdir(parents=True, exist_ok=True)
        import shutil
        for i, img in enumerate(input_images[:5]):  # 최대 5개만
            dst = frames_dir / FilePatterns.frame_name(i)
            if not dst.exists():
                shutil.copy(img, dst)
        print(f"  ✓ 입력 이미지 {min(5, len(input_images))}개를 프레임으로 복사")
    else:
        frame_from_video(video_path, frames_dir)

    # Step 3: 객체 segmentation (이미 마스크가 있으면 건너뛰기)
    existing_masks = list(MASKED_FRAMES_DIR.glob(FilePatterns.MASKED_GLOB))
    if existing_masks:
        print(f"\n[Step 3] SKIP - 기존 마스크 {len(existing_masks)}개 사용")
    elif DEBUG_MODE:
        print(f"\n[Step 3] DEBUG - Segmentation 스킵")
        print(f"  실제 실행 시: SAM 모델로 객체 segmentation")
        # 더미 마스크 이미지 생성 (RGBA)
        MASKED_FRAMES_DIR.mkdir(parents=True, exist_ok=True)
        frames = list(frames_dir.glob("*.png"))
        for frame in frames[:5]:
            mask_path = MASKED_FRAMES_DIR / FilePatterns.masked_name(frame.stem)
            if not mask_path.exists():
                # 원본 이미지를 RGBA로 변환하여 저장 (간단한 더미)
                img = Image.open(frame).convert("RGBA")
                img.save(mask_path)
        print(f"  ✓ 더미 마스크 {min(5, len(frames))}개 생성")
    else:
        segment_objects(frames_dir)

    # Step 4: 적합한 배경 찾기 (캐시 사용)
    if DEBUG_MODE:
        print(f"\n[Step 4] DEBUG - 배경 검색 스킵")
        print(f"  실제 실행 시: {max_backgrounds}개 배경 검색 (semantic matching)")
        # 더미 배경 데이터
        backgrounds = []
        for i in range(min(3, max_backgrounds)):
            # 간단한 더미 배경 (그라데이션)
            dummy_bg = Image.new("RGB", (1024, 1024), (100 + i * 30, 100 + i * 20, 100 + i * 10))
            backgrounds.append({
                "bg_image": dummy_bg,
                "path": f"dummy_bg_{i}.jpg",
                "similarity": 0.9 - i * 0.1
            })
        print(f"  ✓ 더미 배경 {len(backgrounds)}개 생성")
    else:
    ...
    # Step 5: 자연스러운 합성 (이미 결과가 있으면 건너뛰기)
    result_dir = OUTPUT_DIR / "result" / "images" / "train"
    existing_composites = list(result_dir.glob(FilePatterns.COMPOSITE_GLOB)) if result_dir.exists() else []
    if len(existing_composites) >= target_images:
        print(f"\n[Step 5] SKIP - 기존 합성 이미지 {len(existing_composites)}개 (목표: {target_images})")
        output_paths = existing_composites
    elif DEBUG_MODE:
        print(f"\n[Step 5] DEBUG - 합성 스킵")
        print(f"  실제 실행 시: {target_images}개 합성 이미지 생성")
        print(f"  - 객체: {len(objects)}개")
        print(f"  - 배경: {len(backgrounds)}개")
        print(f"  - Depth 사용: {use_depth}")
        print(f"  - Lighting 조정: {use_lighting}")
        output_paths = []
    else:
