...
REPO_ROOT = BASE_DIR.parent / "Depth-Anything-V2"
...
checkpoint_path = BASE_DIR / f"checkpoints/depth_anything_v2_{ENCODER}.pth"
if not checkpoint_path.exists():
    alt_path = REPO_ROOT / f"checkpoints/depth_anything_v2_{ENCODER}.pth"
    if alt_path.exists():
...
def compute_depth(image_pil):
    ...
    depth_color = (depth_cmap(depth_uint8)[:, :, :3] * 255).astype(np.uint8)
