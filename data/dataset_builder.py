...
class DatasetConfig:
    ...
    def __init__(
        ...
        self.OBJ_ROOT = obj_root if isinstance(obj_root, list) else [obj_root]
        self.OUTPUT_ROOT = output_root
        self.TARGET_SIZE = target_size
        self.TILE_SIZE = tile_size
        self.NUM_BG_GROUPS = num_bg_groups
...
class DatasetBuilder:
    def __init__(self, config: DatasetConfig):
        ...
        self.harmonization_net = image_harmonization.ImageHarmonizationModel(device=self.cfg.DEVICE)
        self._setup_folders()
        
        self.object_files = self._load_files(self.cfg.OBJ_ROOT, ('*.png',))
        self.bg_paths = self._load_files([self.cfg.BG_ROOT], ('*.jpg', '*.png', '*.jpeg'))
        
        print(f"📦 객체 파일: {len(self.object_files)}개")
        print(f"🖼️ 배경 파일: {len(self.bg_paths)}개")
    ...
    def _load_files(self, root_dirs, extensions):
        ...
        for root_dir in root_dirs:
            if not os.path.exists(root_dir):
                print(f"⚠️ 경로가 존재하지 않습니다: {root_dir}")
                continue
    ...
    def _get_tiled_backgrounds(self, target_count):
        """타일링된 배경 리스트를 생성합니다."""
        tiles_needed = (self.cfg.TARGET_SIZE // self.cfg.TILE_SIZE) ** 2  # 16 for 1024/256
        total_needed = target_count * tiles_needed
        
        if len(self.bg_paths) < total_needed:
            raise Exception(f"❌ 배경 이미지가 부족합니다! {total_needed}개 필요한데, {len(self.bg_paths)}개밖에 없습니다.")
    ...
    def _generate_data_yaml(self):
        ...
        yaml_path = os.path.join(self.cfg.OUTPUT_ROOT, 'data.yaml')
        
        yaml_content = f"""# YOLO Dataset Configuration
# Auto-generated by DatasetBuilder

path: {os.path.abspath(self.cfg.OUTPUT_ROOT)}
train: images/train
val: images/val
