...
class DependencyChecker:
    ...
    def check(self):
        # DebugMode가 켜져 있으면 heavy 패키지 스킵
        skip_heavy = self.debug_mode.enabled
        required_packages = self.base_packages if skip_heavy else {**self.base_packages, **self.heavy_packages}
...
class DebugMode:
    ...
    def __new__(cls, enabled: bool = None):
        if cls._instance is None:
            cls._instance = super(DebugMode, cls).__new__(cls)
        
        if enabled is not None:
            cls._enabled = bool(enabled)  # setter 없이 바로 클래스 속성 변경
    ...
    def enabled(self, value: bool):
        self.__class__._enabled = bool(value)
