...
# 디렉토리 설정 - 환경 변수로 받거나 기본값 사용
BASE_DIR = Path(__file__).parent                    # servers/
ROOT_DIR = BASE_DIR.parent                          # Image_Analyzer/

# 환경 변수로 경로 받기 (main.py에서 설정)
INPUT_DIR = Path(os.environ.get("POSE_INPUT_DIR", str(ROOT_DIR / "output" / "1_segmented_input")))
POSE_DIR = Path(os.environ.get("POSE_OUTPUT_DIR", str(INPUT_DIR / "pose")))
POSES_FILE = POSE_DIR / "camera_poses.json"         # 포즈 파일
TEMPLATES_DIR = BASE_DIR / "pose_estimator_templates"  # 템플릿 폴더
...
async def complete_and_shutdown():
    """
    작업 완료 신호를 받고 서버 종료
    다운로드 버튼 또는 완료 버튼에서 호출
    """
    import os
    import signal
    
    pose_exists = POSES_FILE.exists()
    print(f"\n🎉 포즈 추정 완료! 포즈 파일: {POSES_FILE}")
    print("--- 서버 종료 중... ---")
    
    # 서버 종료
    os.kill(os.getpid(), signal.SIGTERM)
    
    return JSONResponse({"status": "success", "pose_saved": pose_exists})
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    uvicorn.run("pose_estimator:app", host="0.0.0.0", port=8002, reload=False)